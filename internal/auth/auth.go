...
func secretFreeBackendError(operation, provider string, err error) error {
	if err == nil {
		return nil
	}
	hint := fmt.Sprintf("run `clai auth login --provider %s` to use the private file fallback", provider)
	if env := EnvVarFor(provider); env != "" {
		hint = fmt.Sprintf("set %s or %s", env, hint)
	}
	if roots, rootErr := resolveConfigRoots(); rootErr == nil {
		hint += fmt.Sprintf(" at %q", roots.PreferredPath(fileName))
	}
	return backendError{
		message: fmt.Sprintf("%s: keyring unavailable; %s, then retry after fixing keyring access", operation, hint),
		cause:   err,
	}
}
...
func Resolve(provider, explicit string) (string, error) {
	...
	if fileErr != nil {
		...
		return "", errors.Join(
			secretFreeBackendError("read credential from keyring", provider, keyringErr),
	...
	if !errors.Is(keyringErr, keyring.ErrNotFound) {
		return "", secretFreeBackendError("read credential from keyring", provider, keyringErr)
	}
	return "", nil
}

// StoreResult carries non-fatal follow-up work from a successful StoreDetailed
// call. Warning never changes whether the credential was stored successfully.
type StoreResult struct {
	Warning error
}

// StoreDetailed persists an API key, preferring the OS keyring and falling
// back to a private config file when the keyring is unavailable. It reports
// non-fatal cleanup trouble separately: a successful keyring write remains
// authoritative even when removal of an older file-backed copy fails.
func StoreDetailed(provider, key string) (StoreResult, error) {
	if err := credentialKeyring.Set(serviceName, provider, key); err == nil {
		if cleanupErr := deleteFileEntry(provider); cleanupErr != nil {
			return StoreResult{Warning: fmt.Errorf("credential stored in keyring, but stale file cleanup failed: %w; remove the stale entry from the private credential file and retry login", cleanupErr)}, nil
		}
		return StoreResult{}, nil
	} else if fileErr := writeFile(provider, key); fileErr != nil {
		return StoreResult{}, errors.Join(
			secretFreeBackendError("store credential in keyring", provider, err),
			fmt.Errorf("store credential in config file: %w", fileErr),
		)
	}
	return StoreResult{}, nil
}

// Store is the compatibility wrapper for callers that only distinguish
// success from failure. Use StoreDetailed when a non-fatal warning can be
// shown to the user.
func Store(provider, key string) error {
	_, err := StoreDetailed(provider, key)
	return err
}
...
func Delete(provider string) error {
	...
	if errors.Is(keyringErr, keyring.ErrNotFound) {
		...
		keyringErr = secretFreeBackendError("delete credential from keyring", provider, keyringErr)
	}
...
func SourceWithError(provider, explicit string) (string, error) {
	...
	if fileErr != nil {
		...
		return "", errors.Join(
			secretFreeBackendError("inspect credential keyring", provider, keyringErr),
	...
	if !errors.Is(keyringErr, keyring.ErrNotFound) {
		return "", secretFreeBackendError("inspect credential keyring", provider, keyringErr)
	}
	return "none", nil
}
